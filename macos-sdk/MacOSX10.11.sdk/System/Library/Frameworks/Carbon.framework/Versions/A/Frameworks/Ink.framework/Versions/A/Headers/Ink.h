...
/*
   ...
   mouse input (such as the menubar, the Dock, a windowÕs
   titlebar, scrollbar, or controls in general), the Ink Manager
   will treat the pen as a mouse, and will not produce ink. This
   is known as Òinstant mousingÓ, and the user interface
   elements that force instant mousing are known as Òinstant
   mousersÓ.  When it is determined that the pen down did NOT
   ...
   performed early in the Ink ManagerÕs logic that determines
   ...
   applicationÕs function returns any other value (including
   ...
   data in them.  This permits apps to:
     ...
     - prevent inking over special controls (e.g., a ÒFinished
       WritingÓ button an app might provide for a user to indicate
       she is done entering data into the current window or field)
     - terminate the current phrase (e.g., in response to that
       ÒFinished WritingÓ button)
   ...
   recognition, apps can disable the Ink ManagerÕs processing of
   ...
   or when an application sends whole strokesÕ worth of data
 ...
 *    ÒphraseÓ to decide when to erase onscreen ink and initiate
 ...
 *    control over the Ink ManagerÕs automatic phrase termination can
...
enum {
   ...
   * within the tabletÕs ÒproximityÓ range, and does not resume writing
   ...
   * of the tabletÕs ÒproximityÓ (the range over which the tablet can
 ...
 *    
 *    Some gestures have a defined ÒhotspotÓ that can be used to
 *    determine what the gesture should be applied to.  Used in this
 *    fashion, the gesture is called a ÒtargetedÓ gesture. Because the
 *    ÒtargetÓ is only knowable by the application, all gestures used
 *    in non-ink-aware apps are treated as ÒuntargetedÓ.  Untargeted
 ...
 *    gestureÕs hotspot. 
 *    
 *    If the gesture doesnÕt have a hotspot or that hotspot lies
 *    outside the applicationÕs windows, the gesture should usually be
 *    treated as an untargeted gesture.  An exception is the Clear
 *    gesture, which doesnÕt have a hotspot, but might be targeted to
 *    all of the text it overlaps (to a sufficient degree--say, more
 *    than 50% of each wordÕs onscreen area, on a word by word basis).
 *    
 *    
 ...
 *    or lowercase ÔvÕ. (On the other hand, if the gesture is in a
...
enum {
  ...
  /*
   * Non-tentative gesturesÉ  (All except kInkGestureJoin.)
   */
  ...
  /*
   * kInkGestureEscape corresponds to a single press of the ÒescapeÓ
   ...
   * a single ÒspaceÓ character.  The ÒLeftÓ and ÒRightÓ distinction
   ...
   * to a single ÒreturnÓ (newline) character.  The ÒLeftÓ and ÒRightÓ
   * distinction has only to do with how the gesture was drawn,
   * specifically which way the small angle-bracket-like feature is
   * pointing. 
   * 
   * In general, these ÒLeftÓ and ÒRightÓ variants should be treated
  ...
  /*
   * kInkGestureDelete corresponds to a single press of the ÒdeleteÓ
  ...
  /*
   * Tentative gesturesÉ  (Just kInkGestureJoin.) 
   * 
   * kInkGestureJoin should join two words into a single word, eliding
   * the space between them.  It looks like the letter ÔvÕ.  The
   * affected words would be the ones closest to the top-most points at
   * the left and right tips. Note:  A ÒjoinÓ is the only
   * Òcontext-dependentÓ (aka ÒtentativeÓ) gesture currently defined by
   * Apple. 
   * 
 ...
 *  
 *  Discussion:
 *    Returns the userÕs system-wide setting for where inking and
 *    recognition are allowed: nowhere, anywhere, or only in ink-aware
 *    apps.  The ÒnowhereÓ setting corresponds to the situation in
 *    which the user has turned Ink off entirely; the Ink ManagerÕs
 *    services will not be available in this mode.  The ÒanywhereÓ
 ...
 *    frontmost app. The Òink-aware appsÓ setting requires Ink to be
 *    turned on and the user to have selected this mode from the ÒAllow
 *    me to writeÓ pop-up menu; in this mode the user will only be able
 ...
 *    lie outside the applicationÕs windows.
 *  
 *  Result:
 ...
 *    Òwrite anywhereÓ is turned off (when InkUserWritingMode() ==
 ...
 *    events whose screen locations lie outside the applicationÕs
 *    windows.
 *  
 *  Parameters:
 *    
...
/*
   ...
   tabletÕs proximity or waits a user settable period of time
   ...
   previous inkÑeither horizontally, or on a new line. If the
   ...
   until a special ÒFinished WritingÓ button is pressed,
...
/*
   ...
   take advantage of the Ink ManagerÕs recognition services.  To
...
/*
   ...
   result in a reordering of the InkTextRefÕs alternates
   list, so that the selected item becomes the first item in the
   list. This makes the userÕs choice persistent in internal
   ...
   resultant reordering of the InkTextRefÕs alternates list, the
 ...
 *    those keys were held down for more than 50% of the strokeÕs
 *    points.  The Ink Manager assigns the modifier keys associated
 *    with a phraseÕs first stroke to the entire phrase.  The Ink
 ...
 *    beyond the applicationÕs windows.
 *  
 *  Parameters:
 *    
 *    iTextRef:
 ...
 *    bounds. 
 *    
 *    The inkÕs original points and bounds will be scaled and offset to
